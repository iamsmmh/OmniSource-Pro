...
class ApplicationRepository(BaseRepository[Application]):
    ...
    async def get_apps_paginated(
        ...
        open_source: Optional[Any] = None,
        min_trust: Optional[Any] = None,
        min_quality: Optional[Any] = None,
    ...
    async def get_app_by_id_or_slug(self, identifier: str) -> Optional[OmniStoreApp]:
    ...
    async def get_all_apps(self, limit: Optional[int] = None) -> List[OmniStoreApp]:
    ...
    async def get_orm_by_app_id(self, app_id: str) -> Optional[Application]:
    ...
    async def get_orm_by_slug(self, slug: str) -> Optional[Application]:
    ...
    def _build_filtered_query(
        ...
        open_source: Optional[Any],
        min_trust: Optional[Any],
        min_quality: Optional[Any],
        ...
        if platform:
            query = query.where(
                Application.platforms.any(Platform.platform_type == platform)
            )
        if category:
            query = query.where(
                or_(
        ...
        if license:
            query = query.where(
                Application.license.has(License.spdx_id == license)
            )
        if architecture:
            query = query.where(
                Application.architectures.any(
        ...
        if open_source_bool is True:
            ...
            query = query.where(
                Application.open_source_status == OpenSourceStatus.NOT_OPEN_SOURCE
            )
        ...
        if min_trust is not None:
            try:
                ...
                query = query.where(
                    Application.trust_score.has(
                        TrustScore.normalized_score >= trust_val
                    )
                )
        ...
        if min_quality is not None:
            try:
                ...
                query = query.where(
                    Application.quality_score.has(
                        QualityScore.normalized_score >= quality_val
                    )
                )
            except (TypeError, ValueError):
                pass
