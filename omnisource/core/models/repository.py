...
class RepositoryMetadata(Base):
    ...
    release_notes: Mapped[list[dict]] = mapped_column(JSON, default=list)
    languages: Mapped[dict] = mapped_column(JSON, default=dict)
    package_ecosystems: Mapped[list[str]] = mapped_column(JSON, default=list)
    project_tags: Mapped[list[str]] = mapped_column(JSON, default=list)
